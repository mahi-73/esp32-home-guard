...
const char* password = "mahi@8985";  

IPAddress local_IP(192, 168, 31, 232);
...
static esp_err_t audio_handler(httpd_req_t *req) {
    ...
    while (true) {
        for (int i = 0; i < 256; i++) {
            esp_err_t r = adc2_get_raw(ADC2_CHANNEL_4, ADC_WIDTH_BIT_12, &raw_val);
            if (r != ESP_OK) {
                raw_val = 2048;
            }

            int32_t centered = ((int32_t)raw_val - 2048) * 8;
            if (centered > 32767)  centered = 32767;
            if (centered < -32768) centered = -32768;

            sample_buffer[i] = (int16_t)centered;
            delayMicroseconds(60);
        }

        if (httpd_resp_send_chunk(req, (const char *)sample_buffer, sizeof(sample_buffer)) != ESP_OK) {
            break;
        }
